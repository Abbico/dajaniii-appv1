...
def generate_openrouter_response(query, df, risk, tax, horizon, openrouter_key):
    try:
        ...
        metrics = calculate_portfolio_metrics(df, prices)
        summary = f"""
        Portfolio value: ${metrics['current_value'].sum():,.2f}
        Total gain: ${metrics['unrealized_gain'].sum():,.2f}
        Top performer: {metrics.iloc[metrics['unrealized_gain_percent'].idxmax()]['symbol']} ({metrics.iloc[metrics['unrealized_gain_percent'].idxmax()]['unrealized_gain_percent']:.2f}%)
        Worst performer: {metrics.iloc[metrics['unrealized_gain_percent'].idxmin()]['symbol']} ({metrics.iloc[metrics['unrealized_gain_percent'].idxmin()]['unrealized_gain_percent']:.2f}%)
        """
...
if use_openrouter == "OpenRouter":
    ...
    def call_llm(query, portfolio_data, risk_tolerance, tax_sensitivity, investment_horizon):
        return generate_openrouter_response(query, portfolio_data, risk_tolerance, tax_sensitivity, investment_horizon, openrouter_key)
else:
    openai_key = st.sidebar.text_input("OpenAI API Key", type="password")
    st.session_state.OPENAI_API_KEY = openai_key
    def call_llm(query, portfolio_data, risk_tolerance, tax_sensitivity, investment_horizon):
        client = get_openai_client()
        return generate_chat_response(query, portfolio_data, risk_tolerance, tax_sensitivity, investment_horizon, client)
